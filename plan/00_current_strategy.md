...
| Date | What happened |
|---|---|
| 2026-08-02 | **Cram-night round (zwe101).** Account reassigned to Tapiwa per his note (zwe101 login VERIFIED working, GAITE/E4B, same limits). 6 Day-2 wildcard families from awesome-ioai-tasks live-tested as P3 payloads, all cells executed locally (doc 16): 6/6 verified after recoveries; new failure mode #12 found — **P7 false-PASSes the test-data trap** (pack updated). `research/15` (wildcard recipes) + `research/16` (live test) added; **`plan/IOAI_2026_Cram_Sheet.pdf`** shipped — the printable one-night sheet. 12 msgs spent, 48 left in the hour. |
| 2026-07-22 | **Model gap settled + LOCK IN plan shipped.** The real contest bot (chat.ioai2026.kz, Gemma 4 **E4B** — smaller than the 31B used in drills) was live-tested with 3 frugal messages: playbook carries over (suffix + opener work), plus 3 E4B amendments (a third magic string banning mock objects, run-before-trust on T9 fixes, soft line caps / junk imports). Playbook amended; **`plan/01_lock_in.md`** created — the distilled "only 4 things that matter" + final day-by-day Jul 23 → Aug 8. |
| 2026-07-14 | **Drill #2 executed end-to-end for real** — Gemma 4 (`gemma-4-31b-it`, 2000-token cap, fresh chat per prompt via `tools/gemma_contest.py`) wrote 100% of the HT2 modeling code. Progression: baseline MLP SR 0.21 → Gemma CNN 0.64 → +flip augmentation (×4 data) 0.80 local / **0.84 on-Kaggle val**; independently replayed all 1,600 test action sequences in a reimplemented simulator → test SR 0.788. Kaggle code-competition submit (`-k kernel -v 2`): **public LB 0.83** — clears the ~76% plain-BC community bar, below the ~91% loss-weighting+loop-handling tier (next lever). 3 truncations + 2 bugs, all recovered with playbook moves (prescriptive splitting, T9 traceback paste). Artifacts: `plan/drills/runs/2026-07-14_drill02/`. |
...
| 2026-07-12 | **Drill #1 executed end-to-end for real** — Gemma 4 (`gemma-4-31b-it`, hard 2000-token cap via `tools/gemma_contest.py`, fresh chat per prompt) wrote 100% of the modeling code; notebook trained on Kaggle T4 and **submitted** (notebooks-only competition). **Val mean accuracy 93.5%** (old 93.3 / new 93.6) — clears the ~87% community bar; **public LB 0.80773** (val→LB gap ≈ the known label-noise pairs 3/7 & 11/15 — noise handling is the next lever). 4 bugs, all fixed by Gemma first-try from pasted tracebacks. Run artifacts: `plan/drills/runs/2026-07-12_drill01/`. Key rule finding: the 2000-token cap **includes hidden thinking tokens** — the "Answer immediately, do not deliberate" suffix is mandatory, no alternative model needed. |
